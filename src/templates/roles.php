...
<div class="page-body-wrapper">
    ...
    <div class="main-panel">
        <div class="content-wrapper">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title text-white bg-primary p-2">Roles</h4>
                            ...
                            <tr>
                                ...
                                <td class="pr-2">
                                    <select class="form-control" id="sort_by">
                                        ...
                                        <option value="role_id__desc">Newest First</option>
                                        <option value="role_id__asc">Oldest First</option>
                                    </select>
                                </td>
                                ...
                                <td class="pull-left float-left"><a href="<?php echo $site_url."roles?item_type=".$item_type?>" class="btn btn-primary btn-sm">Reset</a></td>
                                <td class="pr-2">
                                ...
                                <td><a href="<?php echo $site_url."role_form?item_type=".$item_type?>" class="btn btn-primary btn-sm">+</a>
                            </tr>
                        </thead>
                    </table>
...
<script type="text/javascript">
    ...
    function updateStatus(){
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo $site_url;?>routes?action=rolefilter&action2=updatestatus',
            data: {
    ...
    function getItems(page_no = 1){
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo $site_url;?>routes?action=rolefilter',
            data: {
                keyword: $("#keyword").val(),
                sort_by : sort_by,
                item_type: '<?php echo $item_type;?>',
            ...
            success: function (response) {
                ...
                if (items && items.length > 0) {
                    $.each(items, function (index, item) {
                        counter++;
                        let edit_url = `<?php echo $site_url;?>role_form?item_type=role&edit_id=${item.role_id}`;
                        $("#listing_display_data").append(`<tr>
                            
                            <td><input type="checkbox" class="checkbox_css item ml-3 sales_summary_list_checkbox" name="chk[]" id="chk_${item.role_id}" accept="allitems" value="${item.role_id}" /></td>

                            <td><a href="${edit_url}">${item.role_title}</a></td>
