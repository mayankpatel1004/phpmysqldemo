                            ...
                            <tr>
                                ...
                                <td class="pr-2">
                                    <select class="form-control" id="sort_by">
                                        ...
                                        <option value="user_id__desc">Newest First</option>
                                        <option value="user_id__asc">Oldest First</option>
                                    </select>
                                </td>
                                ...
                                <td class="pull-left float-left"><a href="<?php echo $site_url."users?item_type=".$item_type?>" class="btn btn-primary btn-sm">Reset</a></td>
                                <td class="pr-2">
                                ...
                                <td><a href="<?php echo $site_url."user_form?item_type=".$item_type?>" class="btn btn-primary btn-sm">+</a>
                            </tr>
                        </thead>
                    </table>
...
<script type="text/javascript">
    ...
    function updateStatus(){
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo $site_url;?>routes?action=userfilter&action2=updatestatus',
            data: {
    ...
    function getItems(page_no = 1){
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo $site_url;?>routes?action=userfilter',
            data: {
                keyword: $("#keyword").val(),
                sort_by : sort_by,
                item_type: '<?php echo $item_type;?>',
                ...
                if(final_response.message == 'Invalid Token'){
                    ...
                    if (items && items.length > 0) {
                        $.each(items, function (index, item) {
                            let edit_url = `<?php echo $site_url;?>user_form?item_type=user&edit_id=${item.user_id}`;
                            counter++;
                            $("#listing_display_data").append(`<tr>
                                
                                <td><input type="checkbox" class="checkbox_css item ml-3 sales_summary_list_checkbox" name="chk[]" id="chk_${item.user_id}" accept="allitems" value="${item.user_id}" /></td>

                                <td><a href="${edit_url}">${item.user_firstname}</a></td>
